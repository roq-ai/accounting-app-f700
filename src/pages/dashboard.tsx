...
import { getTaxes } from 'apiSdk/taxes';
...
import { ExpenseListPage as ShowCaseList } from 'pages/expenses';
...
function DashboardPage() {
  ...
  const taxStat = useSWR<PaginatedInterface<TaxInterface>>(
    () => `/taxes`,
    () => getTaxes({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Taxes',
      result: taxStat?.data?.totalCount,
      isLoading: taxStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bank Accounts',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`expenses`} passHref legacyBehavior>
                  <Button
